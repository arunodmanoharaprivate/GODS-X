...
show_menu() {
    echo "Welcome to GODS-X Installer/Uninstaller"
    echo "Please choose an option:"
    echo "1. Run GODS-X (Install)"
    echo "2. Uninstall GODS-X"
    echo "3. Exit"
}
...
while true; do
    ...
    case $CHOICE in
        1)
            echo "Proceeding with GODS-X installation..."
            break
            ;;
        2)
            echo "Uninstalling GODS-X..."
            bash <(curl -s https://raw.githubusercontent.com/arunodmanoharaprivate/GODS-X/main/rm-GV.sh)
            echo "GODS-X has been uninstalled."
            exit 0
            ;;
        3)
            echo "Exiting..."
...
echo "Downloading GODS-X version $VERSION..."
if [ "$VERSION" == "latest" ]; then
    DOWNLOAD_URL="https://github.com/arunodmanoharaprivate/GODS-X/archive/refs/heads/main.zip"
else
    DOWNLOAD_URL="https://github.com/arunodmanoharaprivate/GODS-X/archive/refs/tags/$VERSION.zip"
fi

cd /home/$USERNAME
if curl -L "$DOWNLOAD_URL" -o GODS-X.zip; then
    echo "Download successful. Extracting files..."
    unzip -o GODS-X.zip -d /home/$USERNAME
    EXTRACTED_DIR=$(ls /home/$USERNAME | grep "GODS-X-" | head -n 1)
    mv "/home/$USERNAME/$EXTRACTED_DIR" /home/$USERNAME/GODS-X
    rm GODS-X.zip
else
    echo "Failed to download GODS-X version $VERSION. Exiting."
    exit 1
fi

# Verify the templates directory exists
if [ -d "/home/$USERNAME/GODS-X/templates" ]; then
...
mkdir -p /var/lib/GODS-X/certs
sudo chown -R $USERNAME:$USERNAME /var/lib/GODS-X/certs

# Check if valid certificate already exists
if [ -f "/var/lib/GODS-X/certs/$DOMAIN.cer" ] && [ -f "/var/lib/GODS-X/certs/$DOMAIN.cer.key" ]; then
    echo "Valid SSL certificate already exists."
    SSL_CONTEXT="--certfile=/var/lib/GODS-X/certs/$DOMAIN.cer --keyfile=/var/lib/GODS-X/certs/$DOMAIN.cer.key"
else
    echo "Generating SSL certificate..."
    curl https://get.acme.sh | sh -s email=$USERNAME@$SERVER_IP
    ~/.acme.sh/acme.sh --issue --force --standalone -d "$DOMAIN" \
        --fullchain-file "/var/lib/GODS-X/certs/$DOMAIN.cer" \
        --key-file "/var/lib/GODS-X/certs/$DOMAIN.cer.key"
    # Fix ownership of the generated certificates
    sudo chown $USERNAME:$USERNAME /var/lib/GODS-X/certs/$DOMAIN.cer
    sudo chown $USERNAME:$USERNAME /var/lib/GODS-X/certs/$DOMAIN.cer.key
...
if sudo systemctl is-active --quiet GODS-X; then
    echo "Stopping existing GODS-X service..."
    sudo systemctl stop GODS-X
fi

# Create a systemd service to keep the Flask app running with Gunicorn
echo "Setting up systemd service..."
cat > /etc/systemd/system/GODS-X.service <<EOL
[Unit]
Description=GODS-X Web App
After=network.target

[Service]
User=$USERNAME
WorkingDirectory=/home/$USERNAME/GODS-X
ExecStart=/bin/bash -c 'source /home/$USERNAME/GODS-X/venv/bin/activate && exec gunicorn -w 4 -b 0.0.0.0:$PORT $SSL_CONTEXT app:app'
Environment="DB_PATH=/etc/x-ui/x-ui.db"
Restart=always
RestartSec=5
StandardOutput=append:/var/log/GODS-X.log
StandardError=append:/var/log/GODS-X.log
SyslogIdentifier=GODS-X
...
sudo systemctl enable GODS-X
sudo systemctl start GODS-X
